...
def collect_results(data, sim_model, params_opt, roundrep, data_folded):
    #--------------------------------------------------------------------------------------
    ...
    # data_folded: a Boolean (True, False) for whether empirical spectrum is folded or not
    #--------------------------------------------------------------------------------------
    ...
    ll = moments.Inference.ll_multinom(sim_model, data)
    ...
    #calculate theta
    theta = moments.Inference.optimal_sfs_scaling(sim_model, data)
    theta = numpy.around(theta, 4)
    print ("\t\t\tTheta = ", theta)

    if data_folded is True:
...
def Optimize_Routine(data, outfile, model_name, func, rounds, param_number, data_folded=True, reps=None, maxiters=None, folds=None, in_params=None, in_upper=None, in_lower=None, param_labels=" "):
    #--------------------------------------------------------------------------------------
    # Mandatory Arguments =
    ...
    #(7) data_folded: A Boolean value (True or False) indicating whether the empirical data is folded (True) or not (False). Default is True.
    ...
    for r in range(rounds):
        ...
        for rep in range(1, (reps_list[r]+1) ):
            ...
            params_opt = moments.Inference.optimize_log(params_perturbed, data, func,lower_bound=lower_bound, upper_bound=upper_bound, verbose=0, maxiter=maxiters_list[r], output_file = "{}.log.txt".format(model_name))
            print ("\t\t\tOptimized parameters = ", params_opt)

            #simulate the model with the optimized parameters
            sim_model = func(params_opt, data.sample_sizes)
